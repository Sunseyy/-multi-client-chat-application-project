    ...
    if channel == None:
        for client in clients:
            try:
                ...
                print(f"the client {client} is no longer connected")
        ...
        while True:
            ...
            if "/help" in message_decode:  # Check if /help is in the message
                ...
                if len(parts) < 2:  # Ensure the command has a channel name
                    client.send("Usage: /join <channel_name>".encode('ascii'))
                    return
                channel_name = parts[2]  # Get the channel name
                if channel_name not in Channels:  # Check if the channel exists
                    client.send(f"Channel {channel_name} does not exist.".encode('ascii'))
                else:
                    # Check if the channel is at maximum capacity
                    if len(Channels[channel_name]) >= max_users_dict[channel_name]:
                        client.send(f"Channel {channel_name} is full. Max number of users is {max_users_dict[channel_name]}.".encode('ascii'))
                    else:
                        if channel_name in channel_passwords:
                # Request password input from the user
                            client.send("This channel requires a password. Please enter the password.".encode('ascii'))
                            ...
                            if entered_password == channel_passwords[channel_name]:
                                ...
                                client.send(f"Successfully joined channel {channel_name}.".encode('ascii'))
                                ...
                                # Incorrect password
                                client.send("Incorrect password. Please try again.".encode('ascii'))
                        else:
                ...
                if len(parts) < 2:  # Ensure the command has a channel name
                    client.send("Usage: /create <channel_name>".encode('ascii'))
                    return
                channel_name= parts[2]
                if channel_name in Channels:
                    client.send('This channel already exists. If you want to join it, please use /join.'.encode('ascii'))
                else:
                    if current_channel is not None:
                        Channels[current_channel].remove(client)

                    
                    client.send(f'Channel {channel_name} created. Now, please specify the max number of users (between 2 and 100).'.encode('utf-8'))
...
# Receive the message from the client
                    ...
                    if 2 <= max_users <= 100:
                        ...
                        client.send(f"Max users for {channel_name} set to {max_users}. Now, please enter a password for the channel.".encode('ascii'))
                        ...
                        client.send(f"Password for {channel_name} set. You can now join the channel.".encode('ascii'))
                        ...
                        client.send("Invalid number of users. Please specify a number between 2 and 100.".encode('ascii'))
                ...
                if current_channel is None:
                    client.send("You are not currently in any channel.".encode('ascii'))
                else:
                        ...
                        broadcast(f"{nickname} has left the channel {current_channel}.".encode('ascii'), current_channel)
                        
                        
                        client.send(f"You have left the channel {current_channel}.".encode('ascii'))
                ...
                if target_nickname in nicknames:
                    ...
                    target_client.send(f'Private message from {nicknames[clients.index(client)]}: {msg}'.encode('ascii'))
                    
                    # Notify the client that the message was sent
                    client.send(f'Private message to {target_nickname}: {msg}'.encode('ascii'))
                else:
                    client.send(f'User {target_nickname} not found.'.encode('ascii'))
